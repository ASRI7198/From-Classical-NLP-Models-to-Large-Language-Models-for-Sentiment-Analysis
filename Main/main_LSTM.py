...
from Architecture.LSTM import LSTMClassifier
...
if __name__ == "__main__":
    ...
    model = LSTMClassifier(input_size=len(vocabulaire)+1, emb_size=args.EMBEDDING_DIM, hidden_size=args.HIDDEN_SIZE, output_size=args.NUM_CLASSES)
    model = model.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    model = Train(train_loader, val_loader, model)
    Test(model, test_loader)
    torch.save(model.state_dict(), args.PATH_SAVE_LSTM)
